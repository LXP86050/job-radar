...
def build_html(matches: list[dict], threshold: int, total_scanned: int, profile_name: str, total_matches: int) -> str:
    ...
    if total_matches > len(matches):
        summary = f"Showing top {len(matches)} of {total_matches} new matches (score ≥ {threshold}) out of {total_scanned} postings scanned."
    else:
        summary = f"{len(matches)} new match{'es' if len(matches) != 1 else ''} (score ≥ {threshold}) out of {total_scanned} postings scanned."
